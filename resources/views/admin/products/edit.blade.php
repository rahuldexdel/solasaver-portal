<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-3 w-full">
            ...
            <div>
                <h1 class="font-bold text-lg text-gray-900 tracking-tight">Edit Product</h1>
                <p class="text-xs text-gray-400 mt-0.5">{{ $product->name }}</p>
            </div>
        </div>
    </x-slot>

    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="w-full">
        @csrf @method('PUT')
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6 space-y-5">
                        <div>
                            ...
                            <input type="text" name="name" value="{{ old('name', $product->name) }}" required placeholder="e.g. SolaSaver Diverter"
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" placeholder="Short product description shown to customers."
                                      class="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition resize-none">{{ old('description', $product->description) }}</textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="p-6 space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <div class="flex items-center rounded-lg border @error('price') border-red-300 @else border-gray-300 @enderror focus-within:ring-2 focus-within:ring-emerald-500 focus-within:border-emerald-500 transition overflow-hidden">
                                    ...
                                    <input type="number" step="0.01" name="price" value="{{ old('price', $product->price) }}" required placeholder="0.00" class="flex-1 px-3 py-2.5 text-sm outline-none">
                                </div>
                            ...
                            <div>
                                ...
                                <div class="flex items-center rounded-lg border border-gray-300 focus-within:ring-2 focus-within:ring-emerald-500 focus-within:border-emerald-500 transition overflow-hidden">
                                    <span class="px-3 py-2.5 bg-gray-50 text-gray-400 text-sm border-r border-gray-200">$</span>
                                    <input type="number" step="0.01" name="installer_price" value="{{ old('installer_price', $product->installer_price) }}" placeholder="0.00" class="flex-1 px-3 py-2.5 text-sm outline-none">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" required placeholder="0"
                            ...
                            <div>
                                ...
                                <input type="text" name="sku" value="{{ old('sku', $product->sku) }}" placeholder="e.g. SOLSAVE1001"
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-6">
                        ...
                        <div class="mb-4">
                            ...
                            @if($product->image)
                                <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="w-full h-40 object-cover rounded-lg border border-gray-200">
                            @else
                                <div class="w-full h-40 rounded-lg border border-dashed border-gray-300 flex items-center justify-center text-gray-300">
                        ...
                        {{-- Upload new --}}
                        <label class="flex flex-col items-center justify-center w-full border-2 border-dashed border-gray-300 rounded-lg py-6 cursor-pointer hover:bg-gray-50 transition">
                            <svg class="w-7 h-7 text-gray-300 mb-2" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"/></svg>
                            <span class="text-xs text-gray-500">Upload new image</span>
                            <input type="file" name="image" accept="image/*" class="hidden">
                        </label>
                        <p class="text-xs text-gray-400 mt-2">Leave blank to keep the current image.</p>
                    </div>
                </div>
